...
function ConsumptionTooltip({ active, payload }: { active?: boolean; payload?: { payload: GestorRow & { consumo: number }; value: number }[] }) {
  ...
  return <div className="chart-tooltip"><strong>{item.naturezaCodigo}</strong><small>{item.naturezaDescricao}</small><span>{item.consumo.toLocaleString('pt-BR', { maximumFractionDigits: 1 })}% consumido</span><span>NF: {formatCurrency(item.nfEntrada)}</span><span>Limite: {formatCurrency(item.limiteTotal)}</span></div>
}
