...
export function DashboardTimeline({ data, error, isLoading, onRetry }: { data: readonly GestorTimelinePoint[] | null; error: Error | null; isLoading: boolean; onRetry: () => void }) {
  ...
  return <section className="dashboard-timeline" aria-labelledby="timeline-title"><header><h3 id="timeline-title">Evolução Temporal</h3><p>Comparativo mensal de Limite Total, NF Entrada e Saldo Previsto.</p></header><div className="dashboard-timeline-viewport"><ResponsiveContainer width="100%" height="100%"><LineChart data={chartData} margin={{ top: 12, right: 16, left: 8, bottom: 4 }}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="label" tick={{ fontSize: 11 }} /><YAxis tickFormatter={compactCurrency} width={62} tick={{ fontSize: 10 }} /><Tooltip content={<TimelineTooltip />} /><Legend wrapperStyle={{ fontSize: 12 }} /><Line type="monotone" dataKey="limiteTotal" name="Limite Total" stroke="#23618b" strokeWidth={2.5} dot={{ r: 3 }} /><Line type="monotone" dataKey="nfEntrada" name="NF Entrada" stroke="#27805d" strokeWidth={2.5} dot={{ r: 3 }} /><Line type="monotone" dataKey="saldoPrevisto" name="Saldo Previsto" stroke="#c15c5c" strokeWidth={2.5} dot={{ r: 3 }} /></LineChart></ResponsiveContainer></div><div className="timeline-explanation" aria-label="Como interpretar a evolução temporal"><article><span className="dashboard-icon"><DashboardIcon name="wallet" /></span><div><strong>Limite Total</strong><p>Representa o orçamento disponível no período (Lim Original + Contingência OK).</p></div></article><article><span className="dashboard-icon dashboard-icon-green"><DashboardIcon name="cart" /></span><div><strong>NF Entrada</strong><p>Representa o valor realizado no período por meio das notas fiscais de entrada.</p></div></article><article><span className="dashboard-icon dashboard-icon-coral"><DashboardIcon name="trend" /></span><div><strong>Saldo Previsto</strong><p>Limite Total - PC aberto - NF Entrada. Quando negativo, indica que os compromissos superaram o orçamento disponível.</p></div></article></div><small className="timeline-note">Valores consolidados conforme as mesmas regras do modo Mensal do Gestor.</small></section>
}
