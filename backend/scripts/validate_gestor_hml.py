...
class HmlValidationError(RuntimeError):
    """Safe validation-runner error without infrastructure details."""


class InvalidValidationEnvironmentError(HmlValidationError):
    pass


class IncompleteHmlConfigurationError(HmlValidationError):
    pass


@dataclass(frozen=True)
class HmlValidationResult:
...
def _validate_settings(settings: Settings) -> None:
    if settings.app_env is not AppEnvironment.HML:
        raise InvalidValidationEnvironmentError(
            "Validation is allowed only when APP_ENV is hml."
        )
    get_database_config(settings)
    if settings.db_protheus_table_suffix is None:
        raise IncompleteHmlConfigurationError(
            "HML table suffix configuration is incomplete."
        )
...
def format_result(result: HmlValidationResult) -> str:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Validate the Gestor composition safely in HML."
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    except (HmlValidationError, DatabaseError) as error:
        print(f"Validação HML não executada: {error}")
        return 1
    except GestorCompositionError as error:
        print(f"Composição HML inconsistente: {error}")
        return 1
